...
class Labor(db.Model):
    ...
    id_no = db.Column(db.String(50), unique=True, nullable=False)  # Aadhaar / internal ID
    phone_no = db.Column(db.String(15), nullable=False)

    attendances = db.relationship("Attendance", back_populates="labor", lazy=True)

    def __repr__(self):
        return f"<Labor {self.name} ({self.id_no})>"
...
class Attendance(db.Model):
    ...
    def __repr__(self):
        return f"<Attendance {self.labor_id} {self.work_date} {self.labor_type}>"
